 ...
 // Global variables related to the Management units 
	...
	map glob_eruption_engine_params_map <- [
 	...
 	init {
		...
		create people number: 70 {
			color <- #red;
  			location <- any_location_in(one_of(Roads));
		}

		create forces number: 10 {
			color <- #green; 
  			location <- any_location_in(one_of(Roads));
		}
	}
...
//species people parent: human skills: [moving] control: simple_bdi{
	...
	perceive target:human in: other_distance {
	...
	perceive target: human in: view_dist {
	...
	rule belief:new_predicate("hazard") remove_intention:at_target new_desire:in_waiting_area strength:5.0;
	rule emotion:new_emotion("fear" ,new_predicate("hazard")) new_desire:in_waiting_area remove_intention:at_target when: fearful strength:5.0;
	//bool noTarget<-true; //serve a controllare se il target è impostato che comunque è regolato in normal_move
	...
	plan share_information_to_friends intention: share_information instantaneous: true {
		list<human> my_friends <- list<human>((social_link_base where (each.liking > 0)) collect each.agent);
...
species people parent: human{ 
	init {
  	...
  	aspect default {
    	draw triangle(5) color: color border: #red;
    	draw circle(view_dist) color: color border: #red wireframe: true;
	}
	
	}
species forces parent: human{
