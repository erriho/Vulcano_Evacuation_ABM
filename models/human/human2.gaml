...
global {
 ...
 init {
	...
	create hazard number: 1 {location <- any_location_in(one_of(Roads)); // o un punto fisso se vuoi
 //radius <- 10.0; // raggio iniziale in metri
 							}
	create catastrophe;
	create shelter number: 5 { location <- any_location_in(one_of(Roads)); }
...
//quindi la catastrofe pure è un agente
	...
	*/
 	...
 	create people number: 50 { location <- any_location_in(one_of(Roads));
...
species Island {aspect default {draw shape color: #grey;}} 
/* 
species Roads {
	aspect default {
...
species Ferry_Route {aspect default {draw shape color: #blue width: 2#meter;}}
...
//species people parent: human skills: [moving] control: simple_bdi{
	point target;
...
species hazard {
 ...
 aspect default {draw circle(hazard_distance) color: rgb(#gamaorange,0.3) border:#gamaorange depth:5;}
}
species catastrophe{
	init{location <- first(hazard).location;}
	reflex expand {catastrophe_distance <- catastrophe_distance + 2.0; // espansione graduale ogni tick 
	}
	aspect default{draw circle(catastrophe_distance) color: rgb(#gamared,0.4) border:#gamared depth:10;	}
}

species shelter {
	int arrived_people <- 0; // nuovo contatore
	aspect default {draw circle(30) color: rgb(#gamablue,0.8) border: #gamablue depth:10;}
}
...
species socialLinkRepresentation{
	...
	aspect base{draw line([origin,destination],50.0) color: color;}
}
