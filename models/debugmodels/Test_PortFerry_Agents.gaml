		...
		create ferry number: 1 {
			...
			loop port over: Port {
				...
				if port.name = "Porto di Levante" {
					...
					self.destination <- port.location;
					write string(self.destination) + "-" + port.location;
				}
			}
		}
	}
...
species ferry parent: evacuation_vehicle {
	...
	reflex ferry_update {
		...
		if evacuation_mode = true {
			if people_on_board > 0 and ready_to_evacuate = false {
				...
				if location != destination and destination != hub_location {
					do goto target: destination speed: speed on: ferry_network;
		...
		else {do goto target: destination speed: speed on: ferry_network;}
	}
